...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const virtualButtons = document.querySelector('.virtual-buttons');
    ...
    if (isMobile) {
        virtualButtons.style.display = 'flex';
    ...
    function updateCoinDisplays() {
        homeCoinDisplay.textContent = `所持コイン: ${playerCoins}`;
        shopCoinDisplay.textContent = `所持コイン: ${playerCoins}`;
    ...
    function updatePhysicsAndMovement() {
        const isMoving = keyboard['w'] || keyboard['s'] || keyboard['a'] || keyboard['d'] || joystickArea.isMoving;
        ...
        if (joystickArea.isMoving) {
            moveVector.add(direction.clone().multiplyScalar(joystickArea.y));
            moveVector.add(right.clone().multiplyScalar(joystickArea.x));
        } else {
        ...
        // オンライン機能の追加
        if (auth.currentUser) {
            const playerRef = database.ref('players/' + auth.currentUser.uid);
            playerRef.set({
                position: {
    ...
    database.ref('players').on('value', (snapshot) => {
        ...
        if (!playersData) {
            // 誰もログインしていない場合、既存のプレイヤーモデルをすべて削除
        ...
        const currentUids = Object.keys(playersData);
        ...
        for (const uid in playersData) {
            ...
            let otherPlayer = otherPlayers[uid];
    ...
    // ユーザーがログアウトした際にFirebaseからデータを削除
    window.addEventListener('beforeunload', () => {
        if (auth.currentUser) {
            database.ref('players/' + auth.currentUser.uid).remove();
        }
    });

    // モバイル用ジョイスティック
    if (isMobile) {
        joystickArea.isMoving = false;
        joystickArea.x = 0;
        joystickArea.y = 0;
        const maxMove = 75; // ジョイスティックヘッドの最大移動量
        ...
        joystickArea.addEventListener('touchmove', (e) => {
            ...
            const moveRatio = Math.min(distance, maxMove) / maxMove;
            joystickArea.isMoving = true;
            joystickArea.x = Math.cos(angle) * moveRatio;
            joystickArea.y = -Math.sin(angle) * moveRatio; // Y軸は逆
            
            gsap.to(joystickHead, {
                x: joystickArea.x * maxMove,
                y: -joystickArea.y * maxMove,
                duration: 0.1,
                ease: 'power1.out'
            });
        });
